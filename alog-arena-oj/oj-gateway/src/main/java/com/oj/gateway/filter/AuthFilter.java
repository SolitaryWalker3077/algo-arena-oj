...
//        boolean isLogin = redisService.hasKey(getTokenKey(userId));
        ...
        if (url.contains(HttpConstants.SYSTEM_URL_PREFIX) && !UserIdentify.ADMIN.getValue().equals(user.getIdentity())) {
            return unauthorizedResponse(exchange, "令牌验证失败");
        }
    ...
    private Mono<Void> unauthorizedResponse(ServerWebExchange exchange, String msg) {
        log.warn("[鉴权异常处理]请求路径:{}, 原因:{}", exchange.getRequest().getPath(), msg);
